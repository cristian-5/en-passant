...
const THEME = { light: "#ffce9e", dark: "#d18b47", highlight: "rgba(15, 164, 46, 0.3)" };
...
export class Position {
	...
	async picture(perspective: Color = "w", coords = true) {
		...
		if (!coords) return canvas.toBuffer("image/png");
		for (const f of FILES) {
			for (let r = 1; r <= 8; r++) {
				let x = 0, y = 0;
				if (perspective == "w") {
				...
				ctx.font = "bold " + side / 4 + "px 'Calibri', sans-serif";
				const sx = side / 10;
				if (perspective == "w") {
					if (r == 1) ctx.fillText(f, x + sx, y + side - sx);
					if (f == "h") ctx.fillText(`${r}`, x + side - sx - 5, y + 13);
				} else {
					if (r == 8) ctx.fillText(f, x + sx, y + side - sx);
					if (f == "a") ctx.fillText(`${r}`, x + side - sx - 5, y + 13);
				}
			}
		}
...
export class Positions {
	...
	#perspective: Color; #coords: boolean;
	#canvas: EmulatedCanvas2D;

	constructor(perspective: Color = "w", coords = true) {
